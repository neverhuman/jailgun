...
impl JailgunAgentRunRequest {
    pub fn validate_for_config_tabs(&self, config_tabs: u16) -> Result<(), String> {
        ...
        let tabs = self.effective_tabs(config_tabs)?;
        self.effective_max_runtime_seconds()?;
        if let Some(initial_tab_burst) = self.browser.initial_tab_burst {
            if initial_tab_burst == 0 {
                return Err("browser.initial_tab_burst must be positive".into());
            }
            if initial_tab_burst > tabs {
                return Err(format!(
                    "browser.initial_tab_burst must be <= tabs; got {} for {} tabs",
                    initial_tab_burst, tabs
                ));
            }
        }
...
mod tests {
    ...
    #[test]
    fn request_rejects_wrong_interface_version() {
        let mut value = request();
        value.version = JAILGUN_AGENT_INTERFACE_VERSION + 1;
    ...
    #[test]
    fn request_rejects_initial_burst_above_tabs() {
        let mut value = request();
        value.tabs = Some(2);
        value.browser.initial_tab_burst = Some(3);

        assert!(value
            .validate_for_config_tabs(1)
            .unwrap_err()
            .contains("initial_tab_burst"));
    }

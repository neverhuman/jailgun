...
export function collectTarDownloadCandidatesFromDom(
  ...
  for (const element of controls) {
    ...
    if ((!tarSources.length && !texSources.length && !genericArchiveDownload && !genericTexDownload) || !clickable) {
      continue;
    }
    ...
    let score = texSources.length > 0 ? 260 : tarSources.length > 0 ? 200 : 120;
    ...
    candidates.push({
      ...
      fileKind: texSources.length > 0 || genericTexDownload ? 'downloaded-tex' : tarSources.length > 0 || genericArchiveDownload ? 'downloaded-archive' : 'downloaded-file'
    });
  }
